...
用法：
  python3 batch_collect.py                # 全量采集
  python3 batch_collect.py --status       # 采集状态
  python3 batch_collect.py --update-list  # 刷新股票清单
...
def save_daily(snapshot, indices, stats, sectors_info, tops, target_date=None):
    """保存每日全量数据。"""
...
def show_status():
    ...
    for d in dates:
        ...
        if os.path.exists(tf):
            ...
            print(f"   {d}: 交易{stats.get('tradingStocks','?')} ↑{stats.get('upCount','?')}↓{stats.get('downCount','?')}"
...
if __name__ == "__main__":
    ...
    # 保存
    save_daily(snapshot, indices, stats, sectors_info, tops)
